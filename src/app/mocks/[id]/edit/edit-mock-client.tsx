...
import { updateMockEndpoint } from '../../actions';
...
interface EditMockClientProps {
  endpoint: MockEndpoint;
}

export function EditMockClient({ endpoint }: EditMockClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateMockEndpoint(endpoint.id, {
        path: formData.path,
      ...
      router.push(`/mocks/${endpoint.id}`);
      router.refresh();
    } catch (error: any) {
      console.error('Error updating mock endpoint:', error);
      alert('Failed to update mock endpoint: ' + error.message);
