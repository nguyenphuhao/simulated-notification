...
interface ForwardConfig {
  ...
  extractTokenFrom: string | null;
  tokenPath: string | null;
  tokenHeaderName: string | null;
  nextForwardConfigId: string | null;
  timeout: number;
  retryCount: number;
  retryDelay: number;
  enabled: boolean;
...
interface EditForwardConfigClientProps {
  initialConfig: ForwardConfig;
}

export function EditForwardConfigClient({
  initialConfig,
}: EditForwardConfigClientProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [forwardConfigs, setForwardConfigs] = useState<ForwardConfigOption[]>([]);
  const [formData, setFormData] = useState({
    proxyPath: initialConfig.proxyPath,
    method: initialConfig.method,
    targetUrl: initialConfig.targetUrl,
    pathRewrite: initialConfig.pathRewrite || '',
    name: initialConfig.name || '',
    description: initialConfig.description || '',
    addHeaders: initialConfig.addHeaders || '{}',
    removeHeaders: initialConfig.removeHeaders || '[]',
    extractTokenFrom: initialConfig.extractTokenFrom || 'none', // Default to none
    tokenPath: initialConfig.tokenPath || '',
    tokenHeaderName: initialConfig.tokenHeaderName || 'Authorization',
    nextForwardConfigId: initialConfig.nextForwardConfigId || '',
    timeout: initialConfig.timeout,
    retryCount: initialConfig.retryCount,
    retryDelay: initialConfig.retryDelay,
    enabled: initialConfig.enabled,
  });

  // Load forward configs for chaining dropdown (exclude current config)
  useEffect(() => {
    fetch('/api/forward-configs')
      .then((res) => res.json())
      .then((data) => {
        if (data.data) {
          setForwardConfigs(data.data.filter((cfg: ForwardConfigOption) => cfg.id !== initialConfig.id));
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/forward-configs/${initialConfig.id}`, {
        method: 'PATCH',
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Failed to update forward config');
      }

      router.push('/forward-configs');
      router.refresh();
    } catch (error: any) {
      console.error('Error updating forward config:', error);
      alert('Failed to update forward config: ' + error.message);
      setIsSubmitting(false);
    }
  };
